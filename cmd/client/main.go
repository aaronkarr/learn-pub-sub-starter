...
func main() {
	...
	if err != nil {
		log.Fatalf("couldn't subscribe to war declarations queue: %v", err)
	}
	...
	for {
		...
		case "spam":
			...
			if err != nil {
				fmt.Printf("error: could not convert %s to integer", command[1])
				continue
			}
			for i := 0; i < n; i++ {
				...
				if err != nil {
					fmt.Printf("error publishing spam to log: %s\n", err)
				}
			}
			fmt.Printf("Spammed the log %v times\n", n)
		case "quit":
			gamelogic.PrintQuit()
			return
		default:
